...
def select_listings_for_geojson(session):
    engine = session.get_bind()
    metadata.reflect(bind=engine)
    etl_runs = Table("etl_runs", metadata, autoload_with=engine)
    listings = Table("listings", metadata, autoload_with=engine)
    listings_details = Table("listings_details", metadata, autoload_with=engine)
    listings_translations = Table(
        "listings_translations", metadata, autoload_with=engine
    )
...
def select_details_missing_translation(session):
    engine = session.get_bind()
    metadata.reflect(bind=engine)
    listings_details = Table("listings_details", metadata, autoload_with=engine)
    listings_translations = Table(
...
def insert_translations(session, items):
    try:
        engine = session.get_bind()
        metadata.reflect(bind=engine)
        listings_translations = Table(
        ...
        stmt = insert(listings_translations).values(items)

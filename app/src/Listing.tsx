...
const Listing = (props: ListingProps) => {
  ...
  const prevImage = () => {
    setCurrentImageIndex(
      (prevIndex) => (prevIndex - 1 + countImages) % countImages
    )
  }
    ...
    <div key={props.url} className="listing">
      <div className="listing-header-container">
        ...
        <div className="listing-exit-icon">
          <button
            className="listing-exit-icon-button"
            onClick={() => props.appendExitedUrl(props.url)}
          >
            {props.countListings}
          </button>
        </div>
      ...
      <div className="listing-image-container">
        ...
        <div className="listing-image-progress-bars">
          {image_urls.map((_, index) => {
            ...
            return (
              <div
                className={`listing-image-progress-bar ${colorClass}`}
                key={index}
              />
            )
          })}
